...
public class IndexChecker {
    ...
    public void checkSurroundingIndexes(MinefieldButton[][] minefield2DArray, int indexA, int indexB) {
        ...
        if (indexA - 1 >= 0 && indexB - 1 >= 0) {
            this.isTopLeftExist = true;
        }// Top left
        if (indexA - 1 >= 0) {
            this.isTopExist = true;
        } // Top
        if (indexA - 1 >= 0 && indexB + 1 < minefield2DArray[0].length) {
            this.isTopRightExist = true;
        } // Top right
        if (indexB - 1 >= 0) {
            this.isLeftExist = true;
        }// Left
        if (indexB + 1 < minefield2DArray[0].length) {
            this.isRightExist = true;
        } // Right
        if (indexA + 1 < minefield2DArray.length && indexB - 1 >= 0) {
            this.isBottomLeftExist = true;
        }  // Bottom left
        if (indexA + 1 < minefield2DArray.length) {
            this.isBottomExist = true;
        } // Bottom
        if (indexA + 1 < minefield2DArray.length && indexB + 1 < minefield2DArray[0].length) {
            this.isBottomRightExist = true;
        } // Bottom right
    }
